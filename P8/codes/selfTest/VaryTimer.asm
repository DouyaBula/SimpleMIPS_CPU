...
Timer:
    ...
    beq $s3, $t1, ABCDE
    nop
    addi    $t0, $zero, 75000000  # slower to watch (3s)
    ABCDE:
    sw  $t0, 0x7F04($zero)  # set preset
    addi    $t0, $zero, 0xB
    sw  $t0, 0x7F00($zero)  # set ctrl
    add $t1, $zero, $s3     # $t1 for loop condition
    add $t2, $zero, $zero   # $t2: 0 --> 9
    addi    $t3, $zero, 9   # $t3: 9 --> 0
